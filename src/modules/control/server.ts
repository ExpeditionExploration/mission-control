...
export class ControlModuleServer extends Module {
    ...
    async setupMotors() {
        ...
        this.on('wrenchTarget', (wrench: Wrench) => {
            ...
            for (const [pKey, terms] of Object.entries(this.virtualToPhysical)) {
                let sum = 0;
                for (const [virtualKey, scale] of Object.entries(terms)) {
                    sum += (wrench[virtualKey] ?? 0) * scale;
                }
                this.physicalMotors[pKey].setPower(sum);
                this.logger.info(`${this.physicalMotors[pKey].name} power set to ${sum}`);
            }
        });
    }
}
